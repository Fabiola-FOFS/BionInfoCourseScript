####REGRESSION modulo de regressao em Estatistica em R 
#LINEAR UNIVARIABLE
df<-read.table("/Users/home/Desktop/fabiola/bioInfor/Scripts R/SCRIPT REGRESSAO.R",header=TRUE,sep=" ")
head(df)
...
ggplot(df, aes(x=Weight.x, y=Height.x)) + 
  geom_point()+theme_minimal()+
  geom_smooth(method=lm, se=FALSE)+geom_point(size = 2)
  stat_cor(method="spearman")

#2. Normally distributed residuals
model<-lm(Weight.x~Height.x,data=df)
